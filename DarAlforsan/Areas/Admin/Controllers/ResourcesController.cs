...
namespace DarAlforsan.Areas.Admin.Controllers
{
    [Authorize]
    [Area("Admin")]
    public class ResourcesController : Core.UI.BaseController
    {
        public ResourcesController(IConfiguration Config) : base(Config)
        {
        ...
        {
            var service = new Services.Resource();
            ...
            {
                plist.Data = plist.Data.OrderBy(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.List.Resource>();
            }
            else //Order Descending
            {
                plist.Data = plist.Data.OrderByDescending(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.List.Resource>();
            }
        ...
        //-----------------------------------------------------------------------------------------
        [HttpPost]
        public IActionResult Save(ViewModels.Form.Resource model)
        {
            try
            {
                var service = new Services.Resource();
