...
namespace DarAlforsan.Areas.Admin.Controllers
{
    [Authorize]
    [Area("Admin")]
    public class ResourceModuleController : Core.UI.BaseController
    {
        public ResourceModuleController(IConfiguration Config) : base(Config)
        {
        ...
        {
            var service = new Services.ResourceModule();
            ...
            {
                plist.Data = plist.Data.OrderBy(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.ResourceModule>();
            }
            else //Order Descending
            {
                plist.Data = plist.Data.OrderByDescending(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.ResourceModule>();
            }
        ...
        {
            return PartialView("_NewOrEdit", new ViewModels.Form.ResourceModule());
        }
        //-----------------------------------------------------------------------------------------
        [HttpPost]
        public IActionResult Save(ViewModels.Form.ResourceModule model)
        {
            try
            {
                var service = new Services.ResourceModule();
