...
namespace DarAlforsan.Areas.Basic.Controllers
{
    [Authorize]
    [Area("Basic")]
    public class SaidAboutUsController : Core.UI.BaseController
    {
        public SaidAboutUsController(IConfiguration Config) : base(Config)
        {
        ...
        {
            var service = new Services.SaidAboutUs();
            ...
            {
                plist.Data = plist.Data.OrderBy(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.SaidAboutUs>();
            }
            else //Order Descending
            {
                plist.Data = plist.Data.OrderByDescending(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.SaidAboutUs>();
            }
        ...
        {
            return PartialView("_NewOrEdit", new ViewModels.Form.SaidAboutUs());
        }
        //-----------------------------------------------------------------------------------------
        [HttpPost]
        public IActionResult Save(ViewModels.Form.SaidAboutUs model)
        {
            try
            {
                var service = new Services.SaidAboutUs();
