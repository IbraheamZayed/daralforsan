...
namespace DarAlforsan.Areas.Basic.Controllers
{
    [Authorize]
    [Area("Basic")]
    public class StatisticsController : Core.UI.BaseController
    {
        public StatisticsController(IConfiguration Config) : base(Config)
        {
        ...
        {
            var service = new Services.Statistics();
            ...
            {
                plist.Data = plist.Data.OrderBy(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.Statistics>();
            }
            else //Order Descending
            {
                plist.Data = plist.Data.OrderByDescending(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.Statistics>();
            }
        ...
        {
            return PartialView("_NewOrEdit", new ViewModels.Form.Statistics());
        }
        //-----------------------------------------------------------------------------------------
        [HttpPost]
        public IActionResult Save(ViewModels.Form.Statistics model)
        {
            try
            {
                var service = new Services.Statistics();
