...
namespace DarAlforsan.Areas.Basic.Controllers
{
    [Authorize]
    [Area("Basic")]
    public class SchoolFruitsController : Core.UI.BaseController
    {
        public SchoolFruitsController(IConfiguration Config) : base(Config)
        {
        ...
        {
            var service = new Services.SchoolFruits();
            ...
            {
                plist.Data = plist.Data.OrderBy(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.SchoolFruits>();
            }
            else //Order Descending
            {
                plist.Data = plist.Data.OrderByDescending(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.SchoolFruits>();
            }
        ...
        {
            return PartialView("_NewOrEdit", new ViewModels.Form.SchoolFruits());
        }
        //-----------------------------------------------------------------------------------------
        [HttpPost]
        public IActionResult Save(ViewModels.Form.SchoolFruits model)
        {
            try
            {
                var service = new Services.SchoolFruits();
