...
namespace DarAlforsan.Areas.Basic.Controllers
{
    [Authorize]
    [Area("Basic")]
    public class SchoolSystemController : Core.UI.BaseController
    {
        public SchoolSystemController(IConfiguration Config) : base(Config)
        {
        ...
        {
            var service = new Services.SchoolSystem();
            ...
            {
                plist.Data = plist.Data.OrderBy(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.SchoolSystem>();
            }
            else //Order Descending
            {
                plist.Data = plist.Data.OrderByDescending(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.SchoolSystem>();
            }
        ...
        {
            return PartialView("_NewOrEdit", new ViewModels.Form.SchoolSystem());
        }
        //-----------------------------------------------------------------------------------------
        [HttpPost]
        public IActionResult Save(ViewModels.Form.SchoolSystem model)
        {
            try
            {
                var service = new Services.SchoolSystem();
