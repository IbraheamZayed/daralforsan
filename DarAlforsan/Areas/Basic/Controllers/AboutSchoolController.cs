...
namespace DarAlforsan.Areas.Basic.Controllers
{
    [Authorize]
    [Area("Basic")]
    public class AboutSchoolController : Core.UI.BaseController
    {
        public AboutSchoolController(IConfiguration Config) : base(Config)
        {
        ...
        {
            var service = new Services.AboutSchool();
            ...
            {
                plist.Data = plist.Data.OrderBy(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.AboutSchool>();
            }
            else //Order Descending
            {
                plist.Data = plist.Data.OrderByDescending(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.AboutSchool>();
            }
        ...
        {
            return PartialView("_NewOrEdit", new ViewModels.Form.AboutSchool());
        }
        //-----------------------------------------------------------------------------------------
        [HttpPost]
        public IActionResult Save(ViewModels.Form.AboutSchool model)
        {
            try
            {
                var service = new Services.AboutSchool();
