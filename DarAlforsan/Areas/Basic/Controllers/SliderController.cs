...
namespace DarAlforsan.Areas.Basic.Controllers
{
    [Authorize]
    [Area("Basic")]
    public class SliderController : Core.UI.BaseController
    {
        public SliderController(IConfiguration Config) : base(Config)
        {
        ...
        {
            var service = new Services.Slider();
            ...
            {
                plist.Data = plist.Data.OrderBy(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.Slider>();
            }
            else //Order Descending
            {
                plist.Data = plist.Data.OrderByDescending(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.Slider>();
            }
        ...
        {
            return PartialView("_NewOrEdit", new ViewModels.Form.Slider());
        }
        //-----------------------------------------------------------------------------------------
        [HttpPost]
        public IActionResult Save(ViewModels.Form.Slider model)
        {
            try
            {
                var service = new Services.Slider();
