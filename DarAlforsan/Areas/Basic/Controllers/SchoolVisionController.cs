...
namespace DarAlforsan.Areas.Basic.Controllers
{
    [Authorize]
    [Area("Basic")]
    public class SchoolVisionController : Core.UI.BaseController
    {
        public SchoolVisionController(IConfiguration Config) : base(Config)
        {
        ...
        {
            var service = new Services.SchoolVision();
            ...
            {
                plist.Data = plist.Data.OrderBy(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.SchoolVision>();
            }
            else //Order Descending
            {
                plist.Data = plist.Data.OrderByDescending(x => x.GetType().GetProperty(colName).GetValue(x, null)).ToList<ViewModels.Form.SchoolVision>();
            }
        ...
        {
            return PartialView("_NewOrEdit", new ViewModels.Form.SchoolVision());
        }
        //-----------------------------------------------------------------------------------------
        [HttpPost]
        public IActionResult Save(ViewModels.Form.SchoolVision model)
        {
            try
            {
                var service = new Services.SchoolVision();
